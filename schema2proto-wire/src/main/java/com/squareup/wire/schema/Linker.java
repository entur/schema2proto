...
public final class Linker {
	...
	private void addImports(Collection<String> sink, Collection<String> paths, Multimap<String, String> publicImports) {
	...
	void addError(String format, Object... args) {
		...
		for (int i = contextStack.size() - 1; i >= 0; i--) {
			...
			if (context instanceof Rpc) {
				...
				error.append(type != null ? String.format("%s extend %s (%s)", prefix, type, extend.location())
