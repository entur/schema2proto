...
/**
 * A collection of .proto files that describe a set of messages. A schema is <i>linked</i>: each field's type name is resolved to the corresponding type
 * definition.
 *
 * <p>
 * Use {@link SchemaLoader} to load a schema from source files.
 */
public final class Schema {
	...
	/**
	 * Returns the service with the fully qualified name {@code name}, or null if this schema defines no such service.
	 */
	...
	/**
	 * Returns the type with the fully qualified name {@code name}, or null if this schema defines no such type.
	 */
	...
	/**
	 * Returns a wire adapter for the message or enum type named {@code typeName}. The returned type adapter doesn't have model classes to encode and decode
	 * from, so instead it uses scalar types ({@linkplain String}, {@linkplain okio.ByteString ByteString}, {@linkplain Integer}, etc.), {@linkplain Map maps},
	 * and {@linkplain java.util.List lists}. It can both encode and decode these objects. Map keys are field names.
	 *
	 * @param includeUnknown true to include values for unknown tags in the returned model. Map keys for such values is the unknown value's tag name as a
	 *                       string. Unknown values are decoded to {@linkplain Long}, {@linkplain Long}, {@linkplain Integer}, or {@linkplain okio.ByteString
	 *                       ByteString} for {@linkplain com.squareup.wire.FieldEncoding#VARINT VARINT}, {@linkplain com.squareup.wire.FieldEncoding#FIXED64
	 *                       FIXED64}, {@linkplain com.squareup.wire.FieldEncoding#FIXED32 FIXED32}, or
	 *                       {@linkplain com.squareup.wire.FieldEncoding#LENGTH_DELIMITED LENGTH_DELIMITED} respectively.
	 */
